...
grunt.initConfig({
	...
	meta: {
		modules: 'angular.module("ca.angular", [<%= srcModules %>]);',
		tplmodules: 'angular.module("ca.angular.tpls", [<%= tplModules %>]);',
		all: 'angular.module("ca.angular", ["ca.angular.tpls", <%= srcModules %>]);',
		banner: ['/*',
		' * <%= pkg.name %>',
		' * <%= pkg.homepage %>\n',
	...
	jshint: {
		files: ['Gruntfile.js', 'src/**/*.js', '!src/**/*.html.js'],
		options: {
			jshintrc: '.jshintrc'
		}
	},
...
function findModule(name) {
	...
	var module = {
		name: name,
		moduleName: enquote('ca.angular.' + camelcase(name)),
